(()=>{
  ...
  function queryFor(r){
    ...
    const location=String(r.location||'').replace(/\s+/g,' ').trim();
    // Keep LinkedIn discovery intentionally short. Skills are used later by the
    // extension for fit scoring instead of stuffing the search bar with them.
    return [title,location,'"open to work"'].filter(Boolean).join(' ');
  }
