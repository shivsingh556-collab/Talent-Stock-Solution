...
(function loadReportingModule(){
  if (!document.querySelector('link[data-tss-reports]')) {
    ...
    css.rel = 'stylesheet';
    css.href = 'reports-activity.css?v=20260829-stable-restore-v1';
    css.dataset.tssReports = '1';
    document.head.appendChild(css);
  }
  if (!document.querySelector('script[data-tss-reports]')) {
    const script = document.createElement('script');
    script.src = 'reports-activity.js?v=20260829-stable-restore-v1';
    script.async = false;
...
window.addEventListener('load', () => {
  const BUILD = '20260829-stable-restore-v1';
  const addCss = (href) => {
